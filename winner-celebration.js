...
function celebrate(message){if(celebrationActive||!message||message===lastWinnerMessage)return;lastWinnerMessage=message;celebrationActive=true;const result=resultFrom(message);pauseGame();$('modal')?.classList.add('hidden');$('winnerCelebrationMessage').textContent=result.text;overlay.classList.remove('hidden');clearTimeout(celebrationTimer);celebrationTimer=setTimeout(()=>{overlay.classList.add('hidden');celebrationActive=false;if(result.type!=='full-house'&&resumeAfterCelebration&&hostControlsVisible()){const auto=$('autoCallBtn');if(auto&&!auto.disabled&&!/pause/i.test(auto.textContent||''))auto.click();}resumeAfterCelebration=false;window.dispatchEvent(new CustomEvent('skinnersCelebrationFinished',{detail:{type:result.type}}));},4800);}
